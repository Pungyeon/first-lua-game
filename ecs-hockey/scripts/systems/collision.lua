...
local function contains(outer, inner)
	return inner.position.x >= outer.position.x
		...
		and inner.position.x + inner.dimensions.width <= outer.position.x + outer.dimensions.width
		and inner.position.y + inner.dimensions.height <= outer.position.y + outer.dimensions.height
end

local function aabb(a, b)
	return a.position.x < b.position.x + b.collision.width
...
local function handle_rigid_collision(dt, entity, static)
	local predict = {
		position = Vector:new(entity.position.x + (entity.velocity.x * entity.speed * dt), entity.position.y),
...
local function handle_goal_collision(_, entity, goal)
